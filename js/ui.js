...
const UI = (() => {
  ...
  function showOverview() {
    ...
    openModal(`<h3>The Realm — ${esc(Game.dateStr())}</h3>
      ...
      ${rows.map((r) => `<tr><td><span class="chip" style="background:${r.f.color}"></span></td><td class="${r.f.alive ? '' : 'dead'}">${r.f.hasEmperor ? '👑 ' : ''}${esc(r.f.name)}${r.f.id === S.player ? ' (you)' : ''}${(r.f.persona || []).length ? ` <small class="persona">${esc((r.f.persona || []).join(', '))}</small>` : ''}${r.f.phase === 'hegemon' ? ' <small class="persona" title="The largest power: it governs, courts the second power and finishes small neighbours; others band against it">hegemon</small>' : r.f.phase === 'consolidating' ? ' <small class="persona" title="The largest power, pausing to restore order in its restless cities before marching again">consolidating</small>' : Game.leaderHouse() === r.f.id ? ' <small class="persona" title="Holds a third of the map; others will band against him">leading power</small>' : ''}<br><small style="color:var(--muted)">${esc(r.names) || (r.f.alive ? (r.f.guest ? `in exile under ${esc(Game.fname(r.f.host))}` : '') : 'destroyed')}</small></td><td class="num">${r.cities}</td><td class="num">${fmt(r.troops)}</td><td class="num">${fmt(r.gold)}</td><td class="num">${fmt(r.food)}</td><td class="num">${r.offs}</td><td class="num">${r.f.prestige || 0}</td></tr>`).join('')}</table>
